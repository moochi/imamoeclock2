...
- (id) initWithFlipNumberViewCount: (NSUInteger) viewCount;
{
    ...
    if (self)
	{
		...
		self.mFlipNumberViews = [[NSArray alloc] initWithArray: allViews];
		//[allViews release];
...
- (NSUInteger) intValue
{
	NSMutableString* stringValue = [NSMutableString stringWithCapacity: [self.mFlipNumberViews count]];
	for (JDFlipNumberView* view in self.mFlipNumberViews) {
...
- (void) setIntValue: (NSUInteger) newValue
{
	...
	for (int i = 0; i < [stringValue length] && i < [self.mFlipNumberViews count]; i++)
	{
		JDFlipNumberView* view = (JDFlipNumberView*)[self.mFlipNumberViews objectAtIndex: [self.mFlipNumberViews count]-(1+i)];
...
- (void) animateToNextNumber
{
	...
	JDFlipNumberView* view = (JDFlipNumberView*)[self.mFlipNumberViews lastObject];
...
- (void) animateToValue: (NSInteger) value withDuration: (CGFloat) duration
{
	...
	strvalue = [strvalue substringWithRange: NSMakeRange([strvalue length]-[self.mFlipNumberViews count], [self.mFlipNumberViews count])];
...
{
    JDFlipNumberView* view = nil;
	if (self.mFlipNumberViews && [self.mFlipNumberViews count] > 0)
	{
		NSUInteger i, count = [self.mFlipNumberViews count], xWidth = rect.size.width/count;
		for (i = 0; i < count; i++)
		{
			view = [self.mFlipNumberViews objectAtIndex:i];
...
//    LOG(@"newValue: %d", newValue);
	...
	{
		NSUInteger index = [self.mFlipNumberViews indexOfObject: flipNumberView];
		if (index > 0)
		{
			JDFlipNumberView* view = (JDFlipNumberView*)[self.mFlipNumberViews objectAtIndex: index-1];
            if (upwardsEnded) {
	...
	if (flipNumberView == [self.mFlipNumberViews lastObject]) {
...
{
	if(mTargetMode == YES && flipNumberView == [self.mFlipNumberViews lastObject])
	{
